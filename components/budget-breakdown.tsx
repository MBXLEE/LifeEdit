...
export function BudgetBreakdown({budgets,entries,money,categoryLimits={},addBudget,editBudget,deleteBudget,addSpending,editTransaction,deleteTransaction}:{
  ...
  const rows=budgets.map(b=>({...b,limit:categoryLimits[b.category]>0?categoryLimits[b.category]:b.amount,spent:outgoings.filter(t=>t.category===b.category).reduce((s,t)=>s+t.amount,0)}));
  const allocated=rows.reduce((s,b)=>s+b.limit,0),spent=rows.reduce((s,b)=>s+b.spent,0);
  ...
  return <><div className="le-row mb-5"><h2>Pay-cycle spending breakdown</h2><Button onClick={addBudget}><Plus size={16}/>Add budget</Button></div>
    ...
    <Card><div className="le-table-scroll"><table><thead><tr><th>Category</th><th>Limit</th><th>Spent</th><th>Remaining</th><th>Budget used</th><th>Actions</th></tr></thead><tbody>{rows.map(b=><tr key={b.id}><th><button type="button" className="le-table-category" onClick={()=>setSelected(b.category)}>{b.category}</button>{b.limit!==b.amount&&<small className="le-muted">Budget {money(b.amount)}</small>}</th><td>{money(b.limit)}</td><td><button type="button" className="le-table-amount" aria-label={`View ${b.category} spending`} onClick={()=>setSelected(b.category)}>{money(b.spent)}</button></td><td style={{color:b.spent>b.limit?"#c9616b":"#4a926b"}}>{money(b.limit-b.spent)}</td><td><Progress value={b.limit?b.spent/b.limit*100:0}/><small>{b.limit?(b.spent/b.limit*100).toFixed(0):0}%{b.spent>b.limit?` · ${money(b.spent-b.limit)} over limit`:b.spent>=b.limit*.9?" · Near limit":""}</small></td><td><div className="le-inline"><button type="button" className="le-icon" title={`Record ${b.category} spending`} aria-label={`Record ${b.category} spending`} onClick={()=>addSpending(b.category)}><Plus size={16}/></button><RecordActions name={`${b.category} budget`} edit={()=>editBudget(b)} remove={()=>deleteBudget(b.id)}/></div></td></tr>)}</tbody></table></div>{!rows.length&&<Empty title="No budgets for this pay cycle." action="Create budget" onClick={addBudget}/>}</Card>
