...
export function ExploreModules() {
  return <section className="le-explore"><h2>Explore</h2><div>{workspaceLinks.slice(5).map(([label, href, Icon]) => <Link href={href as Route} key={href}><Icon size={20}/><span>{label}</span></Link>)}<Link href="/onboarding"><Pencil size={20}/><span>Life assessment</span></Link><Link href="/login"><LogOut size={20}/><span>Account</span></Link></div></section>;
}
