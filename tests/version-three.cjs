...
(async()=>{
  ...
  try {
    ...
    await visit('/focus');await page.clock.install();await fill('Focus duration (minutes)',1);await fill('Break duration (minutes)',1);await fill('Number of sessions',2);await click('Start');await page.getByRole('navigation',{name:'Main navigation'}).getByRole('link',{name:'Planner',exact:true}).click();await page.clock.fastForward(61000);await page.waitForFunction(()=>JSON.parse(localStorage.getItem('life-edit-preview-v2')).focusPlan.completed===1);assert.equal((await saved()).focusPlan.phase,'break');await page.reload({waitUntil:'domcontentloaded'});await page.locator('.le-content').waitFor();await page.clock.fastForward(121000);await page.waitForFunction(()=>JSON.parse(localStorage.getItem('life-edit-preview-v2')).focusPlan.phase==='finished');assert.equal((await saved()).focus.length,2);
